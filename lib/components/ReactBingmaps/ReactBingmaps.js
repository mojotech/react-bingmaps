	...
	_createClass(ReactBingmaps, [{
		...
		value: function componentWillReceiveProps(nextProps) {
			...
			if (this.props.polylines !== nextProps.polylines) {
				this.setPolylines(nextProps.polylines, mapReference);
			}
		...
		value: function reactBingmaps(props, Microsoft) {
			...
			if (bingmapKey && Microsoft) {
				...
				this.setPolylines(polylines, mapReference);
			...
			if (map[mapReference] && pushPins) {
				...
				for (var pushPinIndex = 0; pushPinIndex < pushPins.length; pushPinIndex++) {
					if (pushPins[pushPinIndex].location && pushPins[pushPinIndex].location[0] && pushPins[pushPinIndex].location[1]) {
						...
						if (pushPins[pushPinIndex].onClick) {
							Microsoft.Maps.Events.addHandler(_pushpin, "click", pushPins[pushPinIndex].onClick);
						}
						if (pushPins[pushPinIndex].onMouseover) {
							Microsoft.Maps.Events.addHandler(_pushpin, "mouseover", pushPins[pushPinIndex].onMouseover);
						}
						if (pushPins[pushPinIndex].onMouseout) {
							Microsoft.Maps.Events.addHandler(_pushpin, "mouseout", pushPins[pushPinIndex].onMouseout);
						}
					}
				}
			}
		...
		key: 'setPolylines',
		value: function setPolylines(polylines, mapReference) {
			if (map[mapReference] && polylines) {
				...
				for (var i = 0; i < polylines.length; i++) {
					...
					var polylineClick = polyline.onClick ? polyline.onClick : null;
					var polylineMouseover = polyline.onMouseover ? polyline.onMouseover : null;
					var polylineMouseout = polyline.onMouseout ? polyline.onMouseout : null;
					...
					if (polylineLocationsAsMapLocations.length !== 0) {
						...
						if (polylineClick) {
							Microsoft.Maps.Events.addHandler(polylineObject, "click", polylineClick);
						}
						if (polylineMouseover) {
							Microsoft.Maps.Events.addHandler(polylineObject, "mouseover", polylineMouseover);
						}
						if (polylineMouseout) {
							Microsoft.Maps.Events.addHandler(polylineObject, "mouseout", polylineMouseout);
						}
					}
				}
			}
...
ReactBingmaps.propTypes = {
	...
	polylines: _propTypes2.default.arrayOf(_propTypes2.default.shape({
